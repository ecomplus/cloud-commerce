 ...
 */
function webhook(defaultOptions) {
    ...
    var ret = {
        ...
        replace: function replace(id, data, config, cb) {
            api.executeHttp(defaultOptions, 'PATCH', this.baseURL + id, data, config, cb);
        },
        eventTypes: function eventTypes(id, config, cb) {
            api.executeHttp(defaultOptions, 'GET', this.baseURL + id + '/event-types', {}, config, cb);
        }
    };
 ...
 */
function webhookEvent(defaultOptions) {
    ...
    function getAndVerify(body, cb) {
        ...
        try {
            ...
            api.executeHttp(defaultOptions, 'GET', baseURL + webhookEventId, {}, function (error, res) {
                if (error) {
                    cb(error, response);
                } else {
                    cb(err, true);
    ...
    function verify(headers, body, webhookId, callback) {
        ...
        api.executeHttp(defaultOptions, 'POST', '/v1/notifications/verify-webhook-signature', payload, callback);
    }
    ...
    var ret = {
        ...
        resend: function resend(id, config, cb) {
            api.executeHttp(defaultOptions, 'POST', this.baseURL + id + '/resend', {}, config, cb);
        }
    };
    ret = generate.mixin(ret, operations);
    return ret;
 ...
 */
function webhookEventType(defaultOptions) {
 ...
 */
function notification(defaultOptions) {
    return {
        webhook: webhook(defaultOptions),
        webhookEvent: webhookEvent(defaultOptions),
        webhookEventType: webhookEventType(defaultOptions)
    };
}
